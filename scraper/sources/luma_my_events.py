...
SOURCE = "luma:my-events"


def fetch_events() -> list[dict]:
    cookie = os.environ.get("LUMA_COOKIE", "")
    if not cookie:
        print(f"  [{SOURCE}] skipping — no LUMA_COOKIE set")
        return []
    ...
    cursor = None
    while True:
        params = {"pagination_limit": 100, "period": "future"}
        if cursor:
            params["pagination_cursor"] = cursor

        resp = requests.get("https://api.luma.com/home/get-events", headers=headers, params=params, timeout=30)
